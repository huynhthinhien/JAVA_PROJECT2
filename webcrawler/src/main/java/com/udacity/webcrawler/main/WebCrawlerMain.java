...
public final class WebCrawlerMain {
    ...
    private void run() {
        ...
        } else { // the result path not empty
        ...
        try {
            ...
            } else { // the profile output path is not empty
                Path profilePath = Path.of(config.getProfileOutputPath());
                profiler.writeData(profilePath);
            }
        } catch (IOException e) {
            throw new RuntimeException("Write the profile data fail. ",e);
        }

    }

    public static void main(String[] args) throws Exception {
        args = new String[]{"src/main/config/sample_config.json"};
        if (args.length != 1) {
