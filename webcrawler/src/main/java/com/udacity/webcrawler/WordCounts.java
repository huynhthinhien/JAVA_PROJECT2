...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    Map<String, Integer> rs = new LinkedHashMap<>();

    // if wordCount is empty then return
    ...
    wordCounts.entrySet().stream().sorted(comparator).forEachOrdered(p -> {
      // when result is equal popular word count then break
        if (rs.size() < popularWordCount) {
          rs.put(p.getKey(), p.getValue());
        }
    });
    return rs;
  }
